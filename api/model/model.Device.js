...
function renameDevice(currentName, newName){

    if(_devices.find((device) => device.getName() === newName))return false //check if not already exist

    _devices.find((device) => device.getName() === currentName)?.setName(newName)
